...
class TransformerModel(nn.Module):
	def __init__(self, config, voc1, voc2, device, logger, EOS_tag = '</s>', SOS_tag = '<s>'):
		...
		if self.config.embedding == 'albert':
    			self.embedding1 = AlbertEncoder(self.config.emb_name, self.device, self.config.freeze_emb)
		elif self.config.embedding == 'bart':
			self.embedding1 = BartEncoder(self.config.emb_name, self.device, self.config.freeze_emb)
		elif self.config.embedding == 'bert':
				self.embedding1 = BertEncoder(self.config.emb_name, self.device, self.config.freeze_emb)
		elif self.config.embedding == 'electra':
			self.embedding1 = ElectraEncoder(self.config.emb_name, self.device, self.config.freeze_emb)
		elif self.config.embedding == 'gpt2':
    			self.embedding1 = GPT2Encoder(self.config.emb_name, self.device, self.config.freeze_emb)
		elif self.config.embedding == 'roberta':
			self.embedding1 = RobertaEncoder(self.config.emb_name, self.device, self.config.freeze_emb)
		elif self.config.embedding == 't5':
				self.embedding1 = T5Encoder(self.config.emb_name, self.device, self.config.freeze_emb)
		elif self.config.embedding == 'xlnet':
				self.embedding1 = XLNetEncoder(self.config.emb_name, self.device, self.config.freeze_emb)
	...
	def forward(self, ques, src, trg):
		...
		# 		[0., 0., 0., -inf],
		# 		[0., 0., 0., 0.],
		if self.config.embedding == 'albert' or self.config.embedding == 'bart' or self.config.embedding == 'bert' or self.config.embedding == 'electra' or self.config.embedding == 'gpt2' or self.config.embedding == 'roberta' or self.config.embedding == 't5' or self.config.embedding == 'xlnet':
	...
	def trainer(self, ques, input_seq1, input_seq2, config, device=None ,logger=None):
		...
		self.loss = self.criterion(output.view(-1, output_dim), input_seq2[1:,:].flatten())
	...
	for epoch in range(1, config.epochs + 1):
		...
		for data in train_dataloader:
			...
			if config.show_train_acc:
				...
				# _, decoder_output = greedy_decode(ques, sent1_var, sent2_var, input_len2, validation=True)
...
def run_validation(config, model, val_dataloader, voc1, voc2, device, logger, epoch_num, validation = True):
	...
	for data in val_dataloader:
		...
		# val_loss, decoder_output = greedy_decode(ques, sent1_var, sent2_var, input_len2, validation=True)
